...
*Setup instructions for a Gitian build of ZHCASH Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZHCASH
...
- [Building ZHCASH Core](#building-zerohour-core)
...
Clone the git repositories for zerohour and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zerohourcash/zerohourcash.git --recursive
```
...
```bash
sudo cp /home/debian/zerohour/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the zerohour repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ZHCASH Core
----------------

To build ZHCASH Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the zerohour repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zerohour/.git/
...
For example:
```bash
URL=https://github.com/laanwj/zerohour.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit zerohour=${COMMIT} --url zerohour=${URL} ../zerohour/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zerohour=${COMMIT} --url zerohour=${URL} ../zerohour/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zerohour=${COMMIT} --url zerohour=${URL} ../zerohour/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-zerohour:
./bin/gbuild --commit zerohour=${COMMIT},cpp-eth-zerohour=${ETHCOMMIT} --url zerohour=${URL},cpp-eth-zerohour=${ETHURL} ../zerohour/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the zerohour git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zerohour/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git https://github.com/zerohourcash/zerohourcash.git

BTCPATH=/some/root/path/zerohour
SIGPATH=/some/root/path/zerohour-detached-sigs

./bin/gbuild --url zerohour=${BTCPATH},signature=${SIGPATH} ../zerohour/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zerohour-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zerohour-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zerohour-osx-build.assert
```
