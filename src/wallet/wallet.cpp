...
CAmount CWallet::GetDebit(const CTransaction& tx, const isminefilter& filter) const
{
    ...
    for (const CTxIn& txin : tx.vin)
    {
        ...
        if (!MoneyRange(nDebit))
            throw std::runtime_error(std::string(__func__) + ": total debit value out of range");
    }
    return nDebit;
}
...
CAmount CWallet::GetCredit(const CTransaction& tx, const isminefilter& filter) const
{
    ...
    for (const CTxOut& txout : tx.vout)
    {
        ...
        if (!MoneyRange(nCredit))
            throw std::runtime_error(std::string(__func__) + ": total credit value out of range");
    }
    return nCredit;
}
...
CAmount CWallet::GetChange(const CTransaction& tx) const
{
    ...
    for (const CTxOut& txout : tx.vout)
    {
        ...
        if (!MoneyRange(nChange))
            throw std::runtime_error(std::string(__func__) + ": total change value out of range");
    }
    return nChange;
}
...
void CWallet::StakeZHCASHs(bool fStake, CConnman* connman)
{
    ::StakeZHCASHs(fStake, this, connman, stakeThread);
}

void CWallet::StartStake(CConnman *connman)
{
    StakeZHCASHs(true, connman);
}
