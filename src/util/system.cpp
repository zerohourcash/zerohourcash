...
const char * const BITCOIN_CONF_FILENAME = "zerohour.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZHCASH
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZHCASH
    // Mac: ~/Library/Application Support/ZHCASH
    // Unix: ~/.zerohour
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZHCASH";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZHCASH";
#else
    // Unix
    return pathRet / ".zerohour";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("ZHCASH Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The ZHCASH Core developers";
    }
