...
std::unique_ptr<ZHCASHState> globalState;
...
const std::string strMessageMagic = "ZHCASH Signed Message:\n";
...
static void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    const int disconnect_height = chainActive.Tip() ? chainActive.Tip()->nHeight + 1 : 0;
    mempool.removeForReorg(pcoinsTip.get(), disconnect_height, STANDARD_LOCKTIME_VERIFY_FLAGS);
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = zerohourDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            ZHCASHTxConverter converter(tx, NULL, NULL, contractflags);
            ExtractZHCASHTX resultConverter;
            if(!converter.extractionZHCASHTransactions(resultConverter)){
            ...
            std::vector<ZHCASHTransaction> zerohourTransactions = resultConverter.first;
            std::vector<EthTransactionParams> zerohourETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(ZHCASHTransaction zerohourTransaction : zerohourTransactions){
                sumGas += zerohourTransaction.gas() * zerohourTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, zerohourTransaction.gasPrice());
                } else {
                    txMinGasPrice = zerohourTransaction.gasPrice();
                }
                VersionVM v = zerohourTransaction.getVersion();
                if(v.format!=0)
                ...
                if(zerohourTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(zerohourTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(zerohourTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += zerohourTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)zerohourTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(zerohourETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > zerohourTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ZHC additional fees, %d delta bytes\n",
...
bool IsInitialBlockDownload()
{
	...
	//LogPrintf("nChainWork: %s, nMinimumChainWork: %s", chainActive.Tip()->nChainWork.ToString(), nMinimumChainWork.ToString());
        return true;
    }
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // zerohour
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // zerohour
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path zerohourDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(zerohourDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(zerohourDir.string(), std::ios::in | std::ios::out);
...
bool ZHCASHTxConverter::extractionZHCASHTransactions(ExtractZHCASHTX& zerohourtx){
    ...
    std::vector<ZHCASHTransaction> resultTX;
    ...
    zerohourtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool ZHCASHTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool ZHCASHTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
ZHCASHTransaction ZHCASHTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    ZHCASHTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = ZHCASHTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = ZHCASHTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t ZHCASHTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // zerohour
    ZHCASHDGP zerohourDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setZHCASHSchedule(zerohourDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = zerohourDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = zerohourDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = zerohourDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // zerohour
...
///////////////////////////////////////////////////////////////////////////////////////// zerohour
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            ZHCASHTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractZHCASHTX resultConvertZHCASHTX;
            if(!convert.extractionZHCASHTransactions(resultConvertZHCASHTX)){
            ...
            if(!CheckMinGasPrice(resultConvertZHCASHTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertZHCASHTX.first, blockGasLimit, pindex->pprev);
            ...
            for(ZHCASHTransaction& qtx : resultConvertZHCASHTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertZHCASHTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertZHCASHTX.first[k].from(), resultConvertZHCASHTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 25000 && !Params().MineBlocksOnDemand()) {
        //sanity check in case an exploit happens that allows new coins to be minted
        if(pindex->nMoneySupply > (uint64_t)(10000000000 + ((pindex->nHeight - 25000) * 10)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // zerohour
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // zerohour
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // zerohour
            globalState->setRootUTXO(oldHashUTXORoot); // zerohour
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // zerohour
        uint32_t sizeBlockDGP = zerohourDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for ZHCASH PoS we need to sync this after every block to ensure txdb is populated for
