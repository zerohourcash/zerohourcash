...
////////////////////////////////////////////////////////////// zerohour
        dev::eth::Ethash::init();		
        boost::filesystem::path pathTemp = fs::temp_directory_path() / strprintf("test_zerohour_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
        boost::filesystem::create_directories(pathTemp);
        const dev::h256 hashDB(dev::sha3(dev::rlp("")));
        globalState = std::unique_ptr<ZHCASHState>(new ZHCASHState(dev::u256(0), ZHCASHState::openDB(pathTemp.string(), hashDB, dev::WithExisting::Trust), pathTemp.string(), dev::eth::BaseState::Empty));
        dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::zerohourTestNetwork)));
