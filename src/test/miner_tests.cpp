...
static void SolveBlock(CBlock& block, const Consensus::Params& consensus)
{
    block.nNonce = 0;
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (unsigned int i = 0; i < COINBASE_MATURITY + 3; ++i)
    {
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            LOCK(cs_main);
            pblock->nVersion = 4; //use version 4 as we enable BIP34, BIP65 and BIP66 since genesis
            pblock->nTime = chainActive.Tip()->GetBlockTime() + chainparams.GetConsensus().nPowTargetSpacing * 2 + 1;
            CMutableTransaction txCoinbase(*pblock->vtx[0]);
            txCoinbase.nVersion = 1;
            txCoinbase.vin[0].scriptSig = CScript();
            txCoinbase.vin[0].scriptSig = CScript() << chainActive.Height()+1 << i;
            ...
            SolveBlock(*pblock, chainparams.GetConsensus());
        }
    ...
    BOOST_CHECK_EQUAL(calculateReward(pblocktemplate->block), GetBlockSubsidy(chainActive.Tip()->nHeight + 1, chainparams.GetConsensus()));
    ...
    CBlockIndex* nextTip = new CBlockIndex();
    nextTip->phashBlock = new uint256(InsecureRand256());
    nextTip->pprev = prevTip;
    nextTip->nHeight = prevTip->nHeight + 1;
    nextTip->BuildSkip();
    chainActive.SetTip(nextTip);
    pcoinsTip->SetBestBlock(nextTip->GetBlockHash());
    SetMockTime(chainActive.Tip()->GetMedianTimePast() + 1);

    BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey));
    BOOST_CHECK_EQUAL(pblocktemplate->block.vtx.size(), 5U);

    chainActive.SetTip(prevTip);
    pcoinsTip->SetBestBlock(prevTip->GetBlockHash());
    delete nextTip->phashBlock;
    delete nextTip;
    SetMockTime(0);
    mempool.clear();
