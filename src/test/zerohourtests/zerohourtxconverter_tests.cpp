...
void checkResult(bool isCreation, std::vector<ZHCASHTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    ZHCASHTxConverter converter(transaction, NULL);
    ExtractZHCASHTX zerohourTx;
    BOOST_CHECK(converter.extractionZHCASHTransactions(zerohourTx));
    std::vector<ZHCASHTransaction> result = zerohourTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(zerohourtxconverter_tests, TestingSetup)
