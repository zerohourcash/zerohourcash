...
inline void initState(){
    ...
    const std::string dirZHCASH = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<ZHCASHState>(new ZHCASHState(dev::u256(0), ZHCASHState::openDB(dirZHCASH, hashDB, dev::WithExisting::Trust), dirZHCASH + "/zerohourDB", dev::eth::BaseState::Empty));
...
inline dev::Address createZHCASHAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline ZHCASHTransaction createZHCASHTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    ZHCASHTransaction txEth;
    if(recipient == dev::Address()){
        txEth = ZHCASHTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = ZHCASHTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<ZHCASHTransaction> txs){
