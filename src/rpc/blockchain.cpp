...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // zerohour
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // zerohour
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // zerohour
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // zerohour
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination zerohourSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(zerohourSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&zerohourSenderAddress);
