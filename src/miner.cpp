...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setZHCASHSchedule(zerohourDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = zerohourDGP.getBlockSize(nHeight);
    minGasPrice = zerohourDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = zerohourDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    ZHCASHTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractZHCASHTX resultConverter;
    if(!convert.extractionZHCASHTransactions(resultConverter)){
    ...
    for(ZHCASHTransaction zerohourTransaction : zerohourTransactions){
        txGas += zerohourTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + zerohourTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(zerohourTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, zerohourTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "zerohourstake";
...
void StakeZHCASHs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
