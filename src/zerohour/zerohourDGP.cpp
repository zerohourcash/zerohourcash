...
void ZHCASHDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool ZHCASHDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule ZHCASHDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t ZHCASHDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t ZHCASHDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t ZHCASHDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t ZHCASHDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool ZHCASHDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void ZHCASHDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void ZHCASHDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void ZHCASHDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void ZHCASHDGP::createParamsInstance(){
...
dev::Address ZHCASHDGP::getAddressForBlock(unsigned int blockHeight){
...
void ZHCASHDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ZHCASHDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ZHCASHDGP::parseStorageOneUint64(uint64_t& value){
...
void ZHCASHDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule ZHCASHDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
