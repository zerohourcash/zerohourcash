...
ZHCASHState::ZHCASHState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = ZHCASHState::openDB(_path + "/zerohourDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ZHCASHState::ZHCASHState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}
...
ResultExecute ZHCASHState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, ZHCASHTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            zerohour::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> ZHCASHState::vins() const // temp
{
...
void ZHCASHState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* ZHCASHState::vin(dev::Address const& _a) const
{
    return const_cast<ZHCASHState*>(this)->vin(_a);
}

Vin* ZHCASHState::vin(dev::Address const& _addr)
{
...
// void ZHCASHState::commit(CommitBehaviour _commitBehaviour)
// {
...
void ZHCASHState::kill(dev::Address _addr)
{
...
void ZHCASHState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void ZHCASHState::deleteAccounts(std::set<dev::Address>& addrs){
...
void ZHCASHState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void ZHCASHState::validateTransfersWithChangeLog(){
