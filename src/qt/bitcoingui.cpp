    ...
    m_network_style(networkStyle)
{
    ...
    zerohourVersionChecker = new ZHCASHVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a ZHCASH address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and zerohour: URIs)"));
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history"), tr("&Transactions"), this);//ZHC_LINE
    ...
    ZRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/zhrctoken"), tr("&ZRC Tokens"), this);
    ZRCTokenAction->setStatusTip(tr("ZRC Tokens (send, receive or add Tokens in list)"));
    ZRCTokenAction->setToolTip(ZRCTokenAction->statusTip());
    ZRCTokenAction->setCheckable(true);
    ZRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(ZRCTokenAction);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your ZHCASH addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified ZHCASH addresses"));
    ...
    openAction->setStatusTip(tr("Open a zerohour: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible ZHCASH command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(ZRCTokenAction, tokenActions);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && zerohourVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(ZHC_RELEASES, ZHC_RELEASES);
            QString message(tr("New version of ZHCASH wallet is available on the ZHCASH source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    ZRCTokenAction->setEnabled(enabled);
...
void BitcoinGUI::gotoSendTokenPage()
{
    ...
    ZRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to ZHCASH network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("ZHCASH"); // default title
