...
bool ToHash160(const std::string& strZHCASHAddress, std::string& strHash160)
{
    CTxDestination zerohourAddress = DecodeDestination(strZHCASHAddress);
    if(!IsValidDestination(zerohourAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&zerohourAddress);
    if(keyid){
...
bool ToZHCASHAddress(const std::string& strHash160, std::string& strZHCASHAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination zerohourAddress = keyid;
    if(IsValidDestination(zerohourAddress)){
        strZHCASHAddress = EncodeDestination(zerohourAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToZHCASHAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToZHCASHAddress(tokenEvent.receiver, tokenEvent.receiver);
