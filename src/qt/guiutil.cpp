...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("zerohour"))
        return false;
...
{
    QString ret = QString("zerohour:%1").arg(info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open zerohour.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ZHCASH.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ZHCASH (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("ZHCASH (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "zerohour.desktop";
    return GetAutostartDir() / strprintf("zerohour-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=ZHCASH\n";
        else
            optionFile << strprintf("Name=ZHCASH (%s)\n", chain);
