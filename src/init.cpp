 ...
 */
static const char* BITCOIN_PID_FILENAME = "zerohourd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/zerohourcash/zerohourcash>";
    const std::string URL_WEBSITE = "<https://zh.cash>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateZHCASH");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path zerohourStateDir = GetDataDir() / "stateZHCASH";
                bool fStatus = fs::exists(zerohourStateDir);
                const std::string dirZHCASH(zerohourStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsZHCASHstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<ZHCASHState>(new ZHCASHState(dev::u256(0), ZHCASHState::openDB(dirZHCASH, hashDB, dev::WithExisting::Trust), dirZHCASH, existsZHCASHstate));
                dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::zerohourMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(zerohourStateDir.string()));
                if (fReset) {
